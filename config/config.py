...
class Path(LoadDataFromJson):
    ...
    DASHBOARD: str = LoadDataFromJson().file['DASHBOARD']
...
class Attributes(LoadDataFromJson):
    ...
    THRESHOLD: float = LoadDataFromJson().attributes['THRESHOLD']
    CONFIDENCE: float = LoadDataFromJson().attributes['CONFIDENCE']
